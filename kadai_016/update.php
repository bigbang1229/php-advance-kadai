 ...
 if (isset($_GET['id'])) {
     try {
         ...
         $stmt_select_genre_codes = $pdo->query($sql_select_genre_codes);
                  ...
                  $genre_code = $stmt_select_genre_codes->fetchAll(PDO::FETCH_COLUMN);
              } catch (PDOException $e) {
                  exit($e->getMessage());
              }
          } else {
                      ...
                      <div class="back">
                                   ...
                                   <form action="update.php?id=<?= $_GET['id'] ?>" method="post" class="registration-form">
                                       <div>
                                           ...
                                           <input type="number" id="stock_quantity" name="stock_quantity" value="<?= $bookt['stock_quantity'] ?>" min="0" max="100000000" required>
                       
                                           <label for="genre_codee">ジャンルコード</label>
                                           <select id="genre_code" name="genre_code" required>
                                               <option disabled selected value>選択してください</option>
                                               <?php
                                               // 配列の中身を順番に取り出し、セレクトボックスの選択肢として出力する
                                               foreach ($genre_codes as $genre) {
