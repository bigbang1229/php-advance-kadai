 ...
 $password = 'root';
  ...
  if (isset($_POST['submit'])) {
    try {
        ...
        $sql_insert = '
            INSERT INTO books (book_code, book_name, price, stock_quantity, genre_code, updated_at)
             ...
             <form action="create.php" method="post" class="registration-form">
                 <div>
                     ...
                     <select id="genre_code" name="genre_code" required>
                         ...
                         foreach ($genres_codes as $genres_code) {
                             echo "<option value='{$genres_code}'>{$genres_code}</option>";
                         }
                         ?>
                     </select>
                 </div>
